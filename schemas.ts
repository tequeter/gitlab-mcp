...
export const ListTagsSchema = z
  .object({
    project_id: z.coerce.string().describe("Project ID or /-separated path"),
    order_by: z
      .enum(["name", "updated", "version"])
      .optional()
      .describe("Return tags ordered by name, updated, or version. Default is updated."),
    sort: z.enum(["asc", "desc"]).optional().describe("Sort direction"),
    search: z.string().optional().describe("Restrict on tag name. You can use ^term and term$ to find tags that begin and end with term. No other regular expressions are supported."),
  })
  .merge(PaginationOptionsSchema);
